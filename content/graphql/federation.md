...
## マイクロサービス化によるモノリス脱却の過渡期を支えられる

### クライアント側の移行コストを抑えることができる (GraphQL / GraphQL Federation)

モノリス分解でクライアントが被る本来の痛みは、「サービスを1つ切り出すたびに API の形が変わり得る」ことにある。
...
なお、この「クライアントは何も変えなくてよい」状態を実際に守っているのは、単一エンドポイントという性質ではなく、次節の破壊的変更チェックの機構である。

### モノリス GraphQL フィールドの解決譲渡がマイクロサービスチームの一存に委ねられる (GraphQL Federation)

モノリス GraphQL Schema からの分割が非常に容易であるということ。
...
モノリス側との協調デプロイは不要で、合成が通る限り、この移譲は新チームの一存で完了する。
...
詳細は末尾「GraphQL Federation が解かないこと」を参照。

### 段階的に新システムにトラフィック移行することができる (GraphQL / GraphQL Federation)

GraphQL を経由することで、クライアントとの契約は GraphQL Schema のみになるため、それより裏側の技術詳細に全く関心を持たなくて済む。
要するに、前提の通り Schema を目標像として設計してあれば腐敗防止層として機能するので、クライアントに知られないまま新システムへの移行が可能になる。
また、Progressive Override という機能を用いることで、譲渡元・譲渡先のトラフィック割合を決定することができるので、GraphQL Field 単位という細かな単位で段階的トラフィック移行をすることができる。
要は GraphQL Field 単位でカナリアリリースが可能になるということ。
...
GraphQL の実行セマンティクスでは、non-null field で発生したエラーは最も近い nullable な祖先まで伝播する。
切り出し対象の field を nullable に設計してあれば、切り出し直後の新サービスが不安定でも、その field が null になるだけで、モノリス由来の残りのレスポンスは返り続ける。

### Schema 駆動開発に乗っかることができる (GraphQL / GraphQL Federation)
...
Federation 固有の上乗せは、合意した Schema がグラフ全体と矛盾しないことを合成チェックで機械検証可能なので、中央チームの調整なしに進められる。

### 他チームが宣言したエンティティに自チームの field を自チームのみの作業で追加することができる (GraphQL Federation)
